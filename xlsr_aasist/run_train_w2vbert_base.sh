...
args=(
  ...
  --earlystop_epoch "${EARLYSTOP_EPOCH:-6}"
  --encoder_lr "${ENCODER_LR:-2e-7}"
  --backend_lr "${BACKEND_LR:-5e-5}"
  --backend_finetune_lr "${BACKEND_FINETUNE_LR:-1e-5}"
  --encoder_trainable_layers "${ENCODER_TRAINABLE_LAYERS:-8}"
  --encoder_warmup_epochs "${ENCODER_WARMUP_EPOCHS:-2}"
