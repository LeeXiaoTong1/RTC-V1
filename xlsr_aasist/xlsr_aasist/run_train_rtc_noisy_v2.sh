...
cd "$SCRIPT_DIR"
CHECKPOINT=${1:?Usage: bash run_train_rtc_noisy_v2.sh CHECKPOINT [extra Python arguments]}
shift
...
args=(
  ...
  --track "${TRACK:-xlsr_aasist_RTC_noisy_v2}" --out_path "${OUT_PATH:-./exp}"
  ...
  --lr "${LR:-1e-6}" --weight_decay 1e-4 --algo "${RAWBOOST_ALGO:-5}"
  --num_workers "${NUM_WORKERS:-8}" --device "${DEVICE:-cuda:0}" --amp "${AMP:-bf16}"
)
python main_train_rtc_noisy_v2.py "${args[@]}" "$@"
