...
def build_parser():
    ...
    parser.add_argument("--encoder_lr", type=float, default=1e-7)
    parser.add_argument("--backend_lr", type=float, default=1e-5)
...
def main():
    ...
    # Stage 1 keeps normal Conformer dropout as regularization but disables
    # stochastic LayerDrop. Pair-consistency stages use fully deterministic SSL
    # forward instead.
    model.ssl_model.model.config.layerdrop = 0.0
    ...
    print(
        f"Stage1 joint fine-tuning from epoch 1: encoder={args.encoder_lr:.2e}, "
        f"AASIST={args.backend_lr:.2e}; LayerDrop=0"
    )
    print(
    ...
    for epoch in range(1, args.num_epochs + 1):
        ...
        reduced = False
        if bad_epochs >= args.lr_patience:
            next_encoder_lr = max(
